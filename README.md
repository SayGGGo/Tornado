...
> Сижу я, делаю реврайтинг чата, уже готово шифрование AES и RSA ключи, паблик и приват.
> Скорее всего сделаю релиз в пятницу утром, или в четверг. В крайнем случае на выходных.
> Там очень большие уязвимости и мне нужно доделать саму систему сообщений, чтение и уведмоления
> сделаю попозже. Короче Comming soon...

> Также планируется огромная админка, пока-что это только планы и зарисовки, но скорее всего смогу
> начать разработку уже после завершение бекенда. Займет эта история около 2 недель если не больше.
> Щяс подумал, скорее всего в админке слишком много фронта будет, придется нейронку юзать((.
---
...
6. [Лицензия и цены](#лицензия-и-цены)
7. [Тесты и автоматический деплой](#тесты-и-автоматический-деплой)
8. [F.A.Q](#faq)
...
| **`test_app.py`**      | Тесты перед деплоем                                                     |
| **`README и другие`**  | Файлы с информацией о проекте                                           |
| **`LICENSE`**          | MIT Лицензия                                                            |
| **`.gitignore`**       | Игнорируемые файлы для Git                                              |
| **`requirements.txt`** | Библиотеки, необходиые для работы приложения                            |
| **`auth/`**            | Логика авторизации и регистрации                                        |
| **`botapi/`**          | Реализация эндпоинтов и хендлеров для работы ботов.                     |
| **`chat/`**            | Основная логика чата                                                    |
| **`config/`**          | Настройки приложения, логгирования и переменных окружения.              |
| **`models/`**          | Модели для DB (SQLAlchemy)                                              |
| **`system/`**          | Основные ендпоинты системы                                              |
| **`telegram/`**        | Модуль для интеграции Telegram                                          |
| **`utils/`**           | Дополнительные утилиты                                                  |
| **`for_rdm/`**         | MD, PNG и SVG файлы для README и разделов                               |
